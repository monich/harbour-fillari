...
DockedPanel {
    ...
    width: (parent && parent.width) ?
        ((Screen.sizeCategory > Screen.Medium || isLandscape) ? Math.round(parent.width * 3 / 4) : parent.width) - 2 * Theme.paddingMedium :
        Screen.width
    ...
    Rectangle {
        ...
        height: Math.max(icon.height, label.height) + icon.y + Theme.paddingLarge
        ...
        Image {
            ...
            y: Math.max(Theme.paddingLarge, (!isLandscape && 'topCutout' in Screen) ? Screen.topCutout.height : 0)
